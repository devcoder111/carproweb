...
<div class="widget widget-styles widget_edit_enabled">
    <h2 class="widget-title text-center content"><?php echo _l('Categories');?></h2>
        ...
        <ul class="nav nav-stacked right_categoriesmenu nav-pills" id="right_categoriesmenu">
            ...
            <li class="<?php echo ($active_first== $item['title']) ? 'active' : '';?>">
                ...
                <?php foreach ($item['childs'] as $child): ?>
                    ...
                    <?php else:?>
                        <a href='<?php echo site_url($lang_code.'/'.config_db_item("results_page_id").'/?search={"v_search_option_'.$treefield_id.'":"'.rawurlencode($item['title'].' - '.$child['title'].' - ').'"}'); ?>'><i class="fa fa-link"></i> <?php _che($child['title']); ?> <span class="item-count">(<?php _che($child['count']);?>)</span></a>
                    <?php endif;?>
                    </li>
                <?php endforeach; ?>
                </ul>

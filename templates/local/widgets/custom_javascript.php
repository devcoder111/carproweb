...
$(document).ready(function() {
 ...
 <?php if(config_item('enable_ajax_url') == true): ?>
            ...
            <?php if(config_item('search_listing_page')&&$page_id!=config_item('search_listing_page')): ?>
                ...
                ?>
                <?php if(config_item('json_url_encoding')!==FALSE):?>
                    window.location.href='<?php echo site_url($lang_code.'/'.config_item('search_listing_page').'/'.url_title_cro($_title, '-', TRUE))?>?search='+JSON.stringify(data).replace("{", '%7B').replace("}", '%7D');
                <?php else:?>
            ...
            if(!$('#results_conteiner').length || $('.redirect_search').length){
                <?php if(config_item('json_url_encoding')!==FALSE):?>
                    window.location.href='<?php echo site_url($lang_code.'/'.config_db_item("results_page_id").'/'.url_title_cro('map', '-', TRUE))?>?search='+JSON.stringify(data).replace("{", '%7B').replace("}", '%7D');
                <?php else:?>
                    window.location.href='<?php echo site_url($lang_code.'/'.config_db_item("results_page_id").'/'.url_title_cro('map', '-', TRUE))?>?search='+JSON.stringify(data);
                <?php endif;?>
                return false;
            }
...
//                        );
  ...
  /* additionals for map */  
    ...
    function RectangleControl(controlDiv2, map) {
        ...
        controlUI.title = '<?php _l('DrawRectangle');?>';
        ...
        controlText.innerHTML = '<strong><?php _l('DrawRectangle');?></strong>';
...
$(document).ready(function(){
    ...
    $('#quickly_search').on('click',function(e){
        ...
        <?php if(config_item('json_url_encoding')!==FALSE):?>
            window.location.href='<?php echo site_url($lang_code.'/'.config_db_item("results_page_id").'/map');?>?search=%7B"v_search_option_smart":"'+cleanerHTML(smart)+'"%7D';
        <?php else:?>
            window.location.href='<?php echo site_url($lang_code.'/'.config_db_item("results_page_id").'/map');?>?search={"v_search_option_smart":"'+cleanerHTML(smart)+'"}';
        <?php endif;?>
